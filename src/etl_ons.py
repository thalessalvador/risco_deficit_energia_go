...
def _infer_datetime_index(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "data" in cols and np.issubdtype(df["data"].dtype, np.datetime64):
        ...
        dt = pd.to_datetime(df["data"].astype(str) + " " + df["hora"].astype(str), errors="coerce")
        ...
        if cand:
            ...
            raise ValueError("Não foi possível inferir coluna de data/hora do arquivo bruto.")
...
def etl_balanco_subsistema_horario(path: Path, out_dir: Path, submercado: str) -> Optional[Path]:
    ...
    if not sub_cols:
        warnings.warn("Coluna de subsistema não encontrada; assumindo arquivo já filtrado para SE/CO.")
        df = df0
    else:
        sub_col = sub_cols[0]
    ...
    if missing:
        warnings.warn(f"Colunas de geração ausentes no balanço (faltando {missing}); arquivo pode ter layout diferente.")
...
def etl_intercambio_horario(path: Path, out_dir: Path, submercado: str) -> Optional[Path]:
    ...
    from_cols = [c for c in df.columns if c in {"de", "from", "origem", "subsistema_de"}]
    to_cols = [c for c in df.columns if c in {"para", "to", "destino", "subsistema_para"}]
    val_cols = [c for c in df.columns if c in {"valor", "mwmed", "mw", "potencia"}]
    if not from_cols or not to_cols or not val_cols:
        warnings.warn("Não foi possível identificar colunas de 'de/para/valor' em Intercâmbio; pulando.")
        return None
...
def etl_ena_diaria(path: Path, out_dir: Path, submercado: str) -> Optional[Path]:
    ...
    for c in df.columns:
        nc = _norm_text(c)
        if "ena" in nc and ("mw" in nc or "mwmed" in nc or "energia" in nc or "valor" in nc):
            val_col = c
            break
    if val_col is None:
...
def etl_ear_diaria(path: Path, out_dir: Path, submercado: str) -> Optional[Path]:
    ...
    for c in df.columns:
        nc = _norm_text(c)
        if "ear" in nc and ("pct" in nc or "%" in c or "percent" in nc or "_" not in nc):
            val_col = c
            break
    if val_col is None:
        if "valor" in df.columns:
...
def _explode_month_to_daily(df: pd.DataFrame, value_col: str, date_col: str = "data") -> pd.DataFrame:
...
def etl_constrained_off_mensal(path: Path, out_dir: Path, fonte: str, submercado: str) -> Optional[Path]:
    ...
    # valor
    val_candidates = [
        c for c in df.columns if any(k in c for k in ["mwh", "energia_nao_gerada", "corte", "restricao"])
    ]
...
def etl_carga(input_path: Path, out_dir: Path, submercado: str) -> Optional[Path]:
    ...
    for c in df.columns:
        nc = _norm_text(c)
        if any(k in nc for k in ["carga", "demanda"]) and any(k in nc for k in ["mw", "mwh", "mwmed", "valor"]):
            val_col = c
            break
    if val_col is None:
        # fallback comum: 'carga_mwh' ou 'valor'
        val_col = "carga_mwh" if "carga_mwh" in df.columns else ("valor" if "valor" in df.columns else None)
    if val_col is None:
...
def maybe_fetch_nasa_power(out_dir: Path, overwrite: bool = False) -> Optional[Path]:
    ...
    try:
        ...
        warnings.warn("Biblioteca 'requests' não instalada; pulando fetch da NASA POWER.")
        return None
...
def main():
    ap = argparse.ArgumentParser(description="ETL ONS/NASA → CSVs diários esperados pelo pipeline.")
    ap.add_argument("--raw-dir", default="data/raw", help="Diretório com arquivos brutos baixados.")
    ap.add_argument("--out-dir", default="data/raw", help="Diretório de saída para CSVs diários padronizados.")
    ap.add_argument("--submercado", default="SE/CO", help="Submercado alvo (ex.: 'SE/CO').")
    ap.add_argument("--fetch-nasa", action="store_true", help="Baixa NASA POWER e salva clima_go_diario.csv.")
    ap.add_argument("--overwrite", action="store_true", help="Sobrescreve arquivos de saída se existirem.")
    args = ap.parse_args()
    ...
    produced["ger_fontes"] = etl_balanco_subsistema_horario(paths["balanco"], out, args.submercado)
    produced["intercambio"] = etl_intercambio_horario(paths["intercambio"], out, args.submercado)
    ...
    produced["cortes_eolica"] = etl_constrained_off_mensal(paths["corte_eolica"], out, "eolica", args.submercado)
    produced["cortes_fv"] = etl_constrained_off_mensal(paths["corte_fv"], out, "fv", args.submercado)
